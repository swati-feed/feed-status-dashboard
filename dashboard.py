...
# ===== PAGE CONFIG =====
st.set_page_config(
    page_title="120- Jumbotail Delivery Tracking",
    page_icon="📊",
    layout="wide",
...
class GoogleSheetData:
    ...
    def fetch_data(self, force_refresh=False):
        ...
        try:
            ...
            sheet2pm_url = "https://docs.google.com/spreadsheets/d/1SJrK568FXninzicEE6iG2Jr3Ns4Z-hH_DN2aEAVa8go/export?format=csv&id=1SJrK568FXninzicEE6iG2Jr3Ns4Z-hH_DN2aEAVa8go&gid=602178604"
            sheet5pm_url = "https://docs.google.com/spreadsheets/d/1SJrK568FXninzicEE6iG2Jr3Ns4Z-hH_DN2aEAVa8go/export?format=csv&id=1SJrK568FXninzicEE6iG2Jr3Ns4Z-hH_DN2aEAVa8go&gid=1068112179"

            response1 = requests.get(sheet1_url, timeout=30)
            response1.raise_for_status()
            df_sheet1 = pd.read_csv(StringIO(response1.text))

            response2 = requests.get(sheet2pm_url, timeout=30)
            response2.raise_for_status()
            df_2pm = pd.read_csv(StringIO(response2.text))

            response3 = requests.get(sheet5pm_url, timeout=30)
            response3.raise_for_status()
            df_5pm = pd.read_csv(StringIO(response3.text))
            ...
            for df in [df_sheet1, df_2pm, df_5pm]:
                if 'Active' in df.columns:
                    def clean_active(val):
                        ...
                        if val_str == 'active':
                            return 'Active'
                        # Check if it's Inactive (handles In_Active, In Active, Inactive)
                        elif 'inactive' in val_str or 'in_active' in val_str or 'in active' in val_str:
                            return 'Inactive'
                        else:
            ...
            for df in [df_sheet1, df_2pm, df_5pm]:
                if 'Master_status' in df.columns:
                    df['Master_status'] = df['Master_status'].apply(
    ...
    if status_filter != "All":
        if 'Active' in df.columns:
            if status_filter == "Active":
                ...
                df = df[df['Active'].astype(str).str.lower().str.strip() == 'inactive']

    # ===== 2. FREQUENCY FILTER =====
    if frequency_filter != "All":
        if 'Feed_frequency' in df.columns:
            freq_clean = df['Feed_frequency'].astype(str).str.strip().str.lower()

            if frequency_filter == "Daily":
                mask = freq_clean.str.contains('daily', na=False) & ~freq_clean.str.contains('daily/weekly', na=False)
            elif frequency_filter == "Daily/Weekly":
                mask = freq_clean.str.contains('daily/weekly', na=False)
            elif frequency_filter == "Weekly":
                mask = freq_clean.str.contains('weekly', na=False) & ~freq_clean.str.contains('daily', na=False)
            else:
                mask = pd.Series([True] * len(df), index=df.index)

            df = df.loc[mask]

    # ===== 3. MASTER STATUS FILTER =====
    if master_status_filter != "All":
        if 'Master_status' in df.columns:
            master_clean = df['Master_status'].astype(str).str.strip().str.lower()

            if master_status_filter == "Done":
                df = df.loc[master_clean == 'done']
            elif master_status_filter == "Pending":
                df = df.loc[master_clean == 'pending']

    return df

def show_empty_message(df, filter_type, filter_value):
    """Show appropriate message when filtered data is empty"""
    if df.empty:
        if filter_type == "master_status":
            if filter_value == "Done":
                st.info("✅ All deliveries are done! No pending records found.")
            elif filter_value == "Pending":
                st.info("⏳ All deliveries are pending! No done records found.")
        elif filter_type == "status":
            if filter_value == "Active":
                st.info("✅ All feeds are active! No inactive feeds found.")
            elif filter_value == "Inactive":
                st.info("ℹ️ No inactive feeds found. All feeds are active.")
        elif filter_type == "frequency":
            if filter_value == "Daily":
                st.info("📅 No daily frequency feeds found.")
            elif filter_value == "Weekly":
                st.info("📅 No weekly frequency feeds found.")
            elif filter_value == "Daily/Weekly":
                st.info("📅 No daily/weekly frequency feeds found.")
        return True
    return False
...
def main():
    ...
    <div class="main-header">
        <div>
            <h1>
                ...
                120- Jumbotail Delivery Tracking
            </h1>
            <div class="subtitle">
    ...
    with st.sidebar:
        ...
        # Apply filters to ALL sheets independently
        df_sheet1_filtered = apply_filters(df_sheet1, status_filter, frequency_filter, master_status_filter)
        df_2pm_filtered = apply_filters(df_2pm, status_filter, frequency_filter, master_status_filter)
        df_5pm_filtered = apply_filters(df_5pm, status_filter, frequency_filter, master_status_filter)

        # ❌ REMOVED THE OVERWRITING CODE ❌
        # Each sheet now keeps its own filtered data
        ...
        # Summary - show counts for ALL sheets
        st.markdown('<hr class="sidebar-divider">', unsafe_allow_html=True)
        st.markdown('''
    ...
    if page_display == "📊 Dashboard":
        if df_sheet1_filtered.empty:
            # Show message instead of warning
        ...
        if df_sheet1_filtered.empty:
            # Show friendly message based on filters
            messages = []
            if master_status_filter != "All":
                messages.append(f"'{master_status_filter}' records")
            if status_filter != "All":
                messages.append(f"'{status_filter}' feeds")
            if frequency_filter != "All":
                messages.append(f"'{frequency_filter}' frequency")

            filter_text = " and ".join(messages) if messages else "matching filters"
            st.info(f"📋 No {filter_text} found in Sheet 1.")
        else:
        ...
        if df_2pm_filtered.empty:
            ...
            # Show specific message for common scenarios
            if master_status_filter == "Pending" and status_filter == "All":
                st.info("✅ All deliveries are done! No pending records found in 2 PM sheet.")
            elif master_status_filter == "Done" and status_filter == "All":
                st.info("⏳ No done records found. All deliveries are pending in 2 PM sheet.")
            elif status_filter == "Inactive":
                st.info("ℹ️ No inactive feeds found in 2 PM sheet. All feeds are active.")
            else:
                st.info(f"📋 No {filter_text} found in 2 PM sheet.")
        else:
        ...
        if df_5pm_filtered.empty:
            ...
            # Show specific message for common scenarios
            if master_status_filter == "Pending" and status_filter == "All":
                st.info("✅ All deliveries are done! No pending records found in 5 PM sheet.")
            elif master_status_filter == "Done" and status_filter == "All":
                st.info("⏳ No done records found. All deliveries are pending in 5 PM sheet.")
            elif status_filter == "Inactive":
                st.info("ℹ️ No inactive feeds found in 5 PM sheet. All feeds are active.")
            else:
                st.info(f"📋 No {filter_text} found in 5 PM sheet.")
        else:
