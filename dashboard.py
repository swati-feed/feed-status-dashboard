...
class GoogleSheetData:
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise Exception(
                        "No valid credentials found. Please add google_creds to secrets or service_account.json file.")

            self.gc = gspread.authorize(creds)

            self.spreadsheet_id = GOOGLE_SHEET_URL.split("/d/")[1].split("/")[0]
    ...
    def fetch_data(self, force_refresh=False):
        ...
        try:
            ...
            sheet2pm = self.spreadsheet.worksheet("2 pm")
            sheet5pm = self.spreadsheet.worksheet("5 pm")

            df_sheet1 = pd.DataFrame(sheet1.get_all_records())
            df_2pm = pd.DataFrame(sheet2pm.get_all_records())
            df_5pm = pd.DataFrame(sheet5pm.get_all_records())
